function [RDMsPaths, slSTCMetadatas, slSpecs] = optimisedMEGSearchlightRDMs_source(meshPaths, slMasks, adjacencyMatrix, STCMetadatas, userOptions)
    ...
    for subject_i = 1:nSubjects
        ...
        for chi = 'LR'
            for t = 1:nWindowPositions
                RDMsFile = ['searchlightRDMs_', thisSubjectName, '_t', t, '-', lower(chi), 'h.mat'];
                RDMsPaths(subject_i, t).(chi) = fullfile(RDMsDir, RDMsFile);
    ...
    parfor subject_i = 1:nSubjects
        ...
        for chi = 'LR'
            ...
            all_files_exist_this_hemi = true;
            for t = 1:nWindowPositions
                if ~exist(RDMsPaths(subject_i, t).(chi), 'file')
                    all_files_exist_this_hemi = false;
                    break;
                end
            end
            
            if all_files_exist_this_hemi && ~overwriteFlag
                ...
                for t = 1:nWindowPositions
                    ...
                    parsave('-v7.3', RDMsPath, 'searchlightRDMs');
